...
const DeckManager = {
  ...
  async loadPolyphones() {
    ...
    try {
      const res = await fetch('./data/polyphones.json?v=42');
      if (res.ok) {
  ...
  async loadLevelIndex() {
    ...
    try {
      const res = await fetch('./data/hsk_level_index.json?v=42');
      if (res.ok) {
  ...
  async loadLevel(version, levelNum) {
    ...
    try {
      ...
      const response = await fetch(`./data/${filePrefix}${levelNum}.json?v=42`);
      if (!response.ok) {
